...
class Mad_miController extends Controller
{
    /**
    ...
    public function index (Request $request)
    {
        ...
        $akmi = Tb_akmi::where('tahun',$tempstruk->tahun_periode)->first();
        if(is_null($akmi))
        {
            foreach ($kab as $kabs) {
                $temp = new Tb_akmi;
        ...
        if ($request->ajax()) {
            $madstruk = Tb_akmi::leftJoin('Kabkota','Kabkota.id','=','Tb_akmi.id_wilayah')
            ->select('Tb_akmi.*','Kabkota.id as idx','Kabkota.nama as nama')->where('tahun',$tempstruk->tahun_periode)->get();
        ...
        return view('pages.madrasah.mid_data',compact('tempstruk'))
