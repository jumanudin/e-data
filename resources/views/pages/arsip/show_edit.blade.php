@extends('layouts.app', ['title' => __('Edit Dokumen Arsip')])
        ...
        <div class="col-lg-6 col-7">
          ...
          <nav aria-label="breadcrumb" class="d-none d-md-inline-block ml-md-4">
            <ol class="breadcrumb breadcrumb-links breadcrumb-dark">
              ...
              <li class="breadcrumb-item active" aria-current="page">Dokumen Edit</li>
            </ol>
          </nav>
        </div>
      </div>
        ...
        <div class="card shadow">
            <div class="card-header border-0">
                    <div class="row align-items-center">
                        <div class="col-8">
                            <h3 class="mb-0">Edit Data Dokumen Arsip</h3>
                        </div>
                    </div>
            </div>
            <div class="card-body">
            {{ Form::open(['url'=>'arsip_trc/edit/'.$data->id, 'class' => 'form-horizontal', 'method'=>'post','enctype'=>'multipart/form-data']) }}
            @csrf
                <input type="hidden" id="user_id" name="user_id" value="{{ $user->username }}">
                <input type="hidden" id="arsip_id" name="arsip_id" value="{{ $data->arsip_id }}">
                ...
                <div class="form-group{{ $errors->has('url') ? ' has-danger' : '' }} row mb-2">
                    <div class="col-md-12">
                        <input class="form-control{{ $errors->has('url') ? ' is-invalid' : '' }}" placeholder="{{ __('URL Cloude') }}" type="text" name="url" value="{{ $data->url }}">
                ...
                <div class="form-group{{ $errors->has('keterangan') ? ' has-danger' : '' }} row mb-2">
                    <div class="col-md-12">
                        <input class="form-control{{ $errors->has('keterangan') ? ' is-invalid' : '' }}" placeholder="{{ __('Keterangan') }}" type="text" name="keterangan" value="{{ $data->keterangan }}">
